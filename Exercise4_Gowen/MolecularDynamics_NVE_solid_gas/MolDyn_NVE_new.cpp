...
{
  cout << endl;
  cout << "========= MULTI-RUN SIMULATION (Routine 1 & Routine 2) =========" << endl;
  cout << endl;

  // ----------------------------- PARAMETERS -----------------------------
  int M = 5000;       // production steps
  ...
  // =====================================================================
  cout << endl;
  cout << "====================== RUNNING ROUTINE 1 ======================" << endl;
  cout << endl;

  string routine = "routine_1_gas";
  string input_file = "input_gas.dat";
  string blocked_stats_path = "./../Results/blocked_stats_gas.dat";

  // Copy input file name to a global variable or use it inside Input()
  Input(input_file);
  if (restart == "true")
    Equilibrate_system(Nequi, T_i, T_f, Nrescales, routine);
  ...
  // =====================================================================
  cout << endl;
  cout << "====================== RUNNING ROUTINE 2 ======================" << endl;
  cout << endl;

  routine = "routine_2_liquid";
  input_file = "input_liquid.dat";
  blocked_stats_path = "./../Results/blocked_stats_liquid.dat";
  ...
  // =====================================================================
  cout << endl;
  cout << "====================== RUNNING ROUTINE 3 ======================" << endl;
  cout << endl;

  routine = "routine_2_solid";
  input_file = "input_solid.dat";
  blocked_stats_path = "./../Results/blocked_stats_solid.dat";
...
void Input(const std::string &input_file) // Prepare all stuff for the simulation
{ 
    ...
    ReadInput >> temp >> npart >> rho;
    vol = static_cast<double>(npart) / rho;
    box = pow(vol, 1.0 / 3.0);
    ReadInput >> rcut >> delta >> nstep >> restart;
    ReadInput.close();

    // Print parameters if starting fresh
    if (restart == "false") {
        cout << "------------- (restart == false) -----------------" << endl << endl;
        cout << "Classic Lennard-Jones fluid        " << endl;
        cout << "Molecular dynamics simulation in NVE ensemble  " << endl << endl;
        cout << "Interatomic potential v(r) = 4 * [(1/r)^12 - (1/r)^6]" << endl << endl;
        cout << "Number of particles = " << npart << endl;
        cout << "Density = " << rho << ", Volume = " << vol << ", Box edge = " << box << endl;
        cout << "Time step = " << delta << ", Number of steps = " << nstep << endl << endl;
    }
    ...
    // Read particle positions and initialize velocities
    if (restart == "true") {
        cout << "------------- (restart == true) -----------------" << endl;
        cout << "Reading initial configurations from config.0" << endl;
        cout << "Saving previous step to old.0" << endl;

        ifstream ReadConf("config.0");
        ofstream SaveConf("old.0", ios::out | ios::trunc);

        double sumv[3] = {0.0, 0.0, 0.0};

        for (int i = 0; i < npart; ++i) {
            ReadConf >> x[i] >> y[i] >> z[i];
            x[i] *= box; y[i] *= box; z[i] *= box;

            vx[i] = rand() / double(RAND_MAX) - 0.5;
            vy[i] = rand() / double(RAND_MAX) - 0.5;
            vz[i] = rand() / double(RAND_MAX) - 0.5;

            sumv[0] += vx[i]; sumv[1] += vy[i]; sumv[2] += vz[i];
        }
        ReadConf.close();

        for (int d = 0; d < 3; ++d) sumv[d] /= npart;

        double sumv2 = 0.0;
        for (int i = 0; i < npart; ++i) {
            vx[i] -= sumv[0]; vy[i] -= sumv[1]; vz[i] -= sumv[2];
        ...
        cout << "Opening config.final and old.final for defining r(t)" << endl;
        ifstream ReadOld("old.final");
        ifstream ReadConf("config.final");

        for (int i = 0; i < npart; ++i) {
            ReadOld >> xold[i] >> yold[i] >> zold[i];
            ReadConf >> x[i] >> y[i] >> z[i];
            xold[i] *= box; yold[i] *= box; zold[i] *= box;
            x[i] *= box; y[i] *= box; z[i] *= box;
        }
        ReadOld.close();
        ReadConf.close();
